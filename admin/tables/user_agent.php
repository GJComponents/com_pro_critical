...
/**
 * User_agent_list Table class
 */
class Pro_criticalTableUser_agent extends JTable
{
	/**
	...
	{
		parent::__construct('#__pro_critical_user_agent', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the User_agent table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New user_agent. A user_agent created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('user_agent', 'Pro_criticalTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_PRO_CRITICAL_USER_AGENT_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_pro_critical.user_agent.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_pro_critical.user_agent.'.(int) $this->$k;
	}
