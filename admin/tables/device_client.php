...
/**
 * Device_client_list Table class
 */
class Pro_criticalTableDevice_client extends JTable
{
	/**
	...
	{
		parent::__construct('#__pro_critical_device_client', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Device_client table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New device_client. A device_client created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('device_client', 'Pro_criticalTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_PRO_CRITICAL_DEVICE_CLIENT_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_pro_critical.device_client.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_pro_critical.device_client.'.(int) $this->$k;
	}
