...
/**
 * Admins_list Table class
 */
class Pro_criticalTableAdmins extends JTable
{
	/**
	...
	{
		parent::__construct('#__pro_critical_admins', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Admins table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New admins. A admins created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('admins', 'Pro_criticalTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_PRO_CRITICAL_ADMINS_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_pro_critical.admins.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_pro_critical.admins.'.(int) $this->$k;
	}
