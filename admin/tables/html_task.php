...
/**
 * Html_task_list Table class
 */
class Pro_criticalTableHtml_task extends JTable
{
	/**
	...
	{
		parent::__construct('#__pro_critical_html_task', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Html_task table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New html_task. A html_task created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('html_task', 'Pro_criticalTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_PRO_CRITICAL_HTML_TASK_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_pro_critical.html_task.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_pro_critical.html_task.'.(int) $this->$k;
	}
