...
/**
 * Js_style_list Table class
 */
class Pro_criticalTableJs_style extends JTable
{
	/**
	...
	{
		parent::__construct('#__pro_critical_js_style', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Js_style table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New js_style. A js_style created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('js_style', 'Pro_criticalTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_PRO_CRITICAL_JS_STYLE_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_pro_critical.js_style.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_pro_critical.js_style.'.(int) $this->$k;
	}
