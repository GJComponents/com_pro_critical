...
/**
 * Css_file_list Table class
 */
class Pro_criticalTableCss_file extends JTable
{
	/**
	...
	{
		parent::__construct('#__pro_critical_css_file', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Css_file table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New css_file. A css_file created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('css_file', 'Pro_criticalTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_PRO_CRITICAL_CSS_FILE_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_pro_critical.css_file.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_pro_critical.css_file.'.(int) $this->$k;
	}
