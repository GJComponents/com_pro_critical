...
/**
 * Cache_list Table class
 */
class Pro_criticalTableCache extends JTable
{
	/**
	...
	{
		parent::__construct('#__pro_critical_cache', 'id', $db);
	}	
	...
	/**
	 * Overload the store method for the Cache table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New cache. A cache created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('cache', 'Pro_criticalTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_PRO_CRITICAL_CACHE_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_pro_critical.cache.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_pro_critical.cache.'.(int) $this->$k;
	}
