 ...
 * Cache Controller
 */
class Pro_criticalControllerCache extends JControllerForm
{
	/**
	...
	{
		$this->view_list = 'Cache_list'; // safeguard for setting the return view listing to the main view.
		parent::__construct($config);
	}

#Custom Buttons PHP (controller methods) [cache]
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit', 'com_pro_critical.cache.' . (int) $recordId);
			if (!$permission)
			{
				if ($user->authorise('core.edit.own', 'com_pro_critical.cache.' . $recordId))
				{
	...
	public function batch($model = null)
	{
		...
		// Set the model
		$model = $this->getModel('Cache', '', array());

		// Preset the redirect
		$this->setRedirect(JRoute::_('index.php?option=com_pro_critical&view=cache_list' . $this->getRedirectToListAppend(), false));
