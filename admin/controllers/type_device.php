 ...
 */
class Pro_criticalControllerType_device extends JControllerForm
{
	/**
	...
	{
		$this->view_list = 'Type_device_list'; // safeguard for setting the return view listing to the main view.
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit', 'com_pro_critical.type_device.' . (int) $recordId);
			if (!$permission)
			{
				if ($user->authorise('core.edit.own', 'com_pro_critical.type_device.' . $recordId))
				{
	...
	public function batch($model = null)
	{
		...
		// Set the model
		$model = $this->getModel('Type_device', '', array());

		// Preset the redirect
		$this->setRedirect(JRoute::_('index.php?option=com_pro_critical&view=type_device_list' . $this->getRedirectToListAppend(), false));
