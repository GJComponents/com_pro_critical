...
/**
 * Pro_critical Css Model
 */
class Pro_criticalModelCss extends JModelAdmin
{
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_pro_critical.css';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'css', $prefix = 'Pro_criticalTable', $config = array())
	{
...
#Custom Buttons PHP (model methods)[css]
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_pro_critical.css');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_pro_critical.css', 'css', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_pro_critical.css.' . (int) $id))
	...
	{
		return 'administrator/components/com_pro_critical/models/forms/css.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_pro_critical.css.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit.state', 'com_pro_critical.css.' . (int) $recordId);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_pro_critical.css.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_pro_critical.edit.css.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// run the perprocess of the data
			$this->preprocessData('com_pro_critical.css', $data);
		}
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		while (!empty($pks))
		{
			...
			list($this->table->query_string, $this->table->alias) = $this->_generateNewTitle($this->table->alias, $this->table->query_string);
	...
	public function save($data)
	{
		...
		// [Interpretation 6124] Alter the query_string for save as copy
		if ($input->get('task') === 'save2copy')
		{
			$origTable = clone $this->getTable();
			$origTable->load($input->getInt('id'));

			if ($data['query_string'] == $origTable->query_string)
			{
				list($query_string, $alias) = $this->_generateNewTitle($data['alias'], $data['query_string']);
		...
		// [Interpretation 6168] Automatic handling of alias for empty fields
		if (in_array($input->get('task'), array('apply', 'save', 'save2new')) && (int) $input->get('id') == 0)
		{
			if ($data['alias'] == null || empty($data['alias']))
			{
				...
				{
					$data['alias'] = JFilterOutput::stringURLUnicodeSlug($data['query_string']);
				}
				else
				{
					$data['alias'] = JFilterOutput::stringURLSafe($data['query_string']);
				}

				$table = JTable::getInstance('css', 'pro_criticalTable');

				if ($table->load(array('alias' => $data['alias'])) && ($table->id != $data['id'] || $data['id'] == 0))
				{
					$msg = JText::_('COM_PRO_CRITICAL_CSS_SAVE_WARNING');
				}
	...
	protected function _generateNewTitle($alias, $title = null)
	{
		...
		// [Interpretation 6258] Alter the title/s & alias
		$table = $this->getTable();

		while ($table->load(array('alias' => $alias)))
		{
			// [Interpretation 6262] Check if this is an array of titles
			if (Pro_criticalHelper::checkArray($title))
			{
			...
			// [Interpretation 6270] Make sure we have a title
			elseif ($title)
			{
		...
		// [Interpretation 6277] Check if this is an array of titles
		if (Pro_criticalHelper::checkArray($title))
		{
			$title[] = $alias;
			return $title;
		}
		// [Interpretation 6283] Make sure we have a title
		elseif ($title)
		{
			return array($title, $alias);
		}
		// [Interpretation 6288] We only had an alias
		return $alias;
	}
}
