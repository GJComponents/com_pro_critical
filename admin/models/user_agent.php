...
/**
 * Pro_critical User_agent Model
 */
class Pro_criticalModelUser_agent extends JModelAdmin
{
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_pro_critical.user_agent';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'user_agent', $prefix = 'Pro_criticalTable', $config = array())
	{
...
#OnBtnCleanAllTable PHP (model methods)
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_pro_critical.user_agent');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_pro_critical.user_agent', 'user_agent', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_pro_critical.user_agent.' . (int) $id))
	...
	{
		return 'administrator/components/com_pro_critical/models/forms/user_agent.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_pro_critical.user_agent.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit.state', 'com_pro_critical.user_agent.' . (int) $recordId);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_pro_critical.user_agent.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__pro_critical_user_agent'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_pro_critical.edit.user_agent.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// run the perprocess of the data
			$this->preprocessData('com_pro_critical.user_agent', $data);
		}
	...
	{
#PHP publish Method afterTarget (array) $pks is an array of record primary keys and $value is the published state. *
