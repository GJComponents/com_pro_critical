...
/**
 * Typebrowserid Form Field class for the Pro_critical component
 */
class JFormFieldTypebrowserid extends JFormFieldList
{
	/**
	 * The typebrowserid field type.
	 *
	 * @var		string
	 */
	public $type = 'typebrowserid';
	...
	protected function getInput()
	{
		...
		if ($set_button === 'true')
		{
			...
			// [Fields 4857] only add if user allowed to create type_browser
			if ($user->authorise('core.create', 'com_pro_critical') && $app->isAdmin()) // TODO for now only in admin area.
			{
				...
				$button[] = '<a id="'.$button_code_name.'Create" class="btn btn-small btn-success hasTooltip" title="'.JText::sprintf('COM_PRO_CRITICAL_CREATE_NEW_S', $button_label).'" style="border-radius: 0px 4px 4px 0px; padding: 4px 4px 4px 7px;"
					href="index.php?option=com_pro_critical&amp;view=type_browser&amp;layout=edit'.$ref.'" >
					<span class="icon-new icon-white"></span></a>';
			}
			// [Fields 4893] only add if user allowed to edit type_browser
				...
				$script[] = "
					...
					function ".$button_code_name."Button(value) {
						if (value > 0) {
							...
							var url = 'index.php?option=com_pro_critical&view=type_browser_list&task=type_browser.edit&id='+value+'".$refJ."';
			...
			// [Fields 4968] check if button was created for type_browser field.
...
// Get the user object.
		...
		$query->select($db->quoteName(array('a.id','a.type'),array('id','type_browser_id_type')));
		$query->from($db->quoteName('#__pro_critical_type_browser', 'a'));
		$query->where($db->quoteName('a.published') . ' = 1');
		$query->order('a.type ASC');
		// Implement View Level Access (if set in table)
		if (!$user->authorise('core.options', 'com_pro_critical'))
		{
			$columns = $db->getTableColumns('#__pro_critical_type_browser');
		...
		if ($items)
		{
			...
			{
				$options[] = JHtml::_('select.option', $item->id, $item->type_browser_id_type);
			}
		}
		return $options;
	}
