...
/-------------------------------------------------------------------------------------------------------/
	...
	@package		proCritical
	@subpackage		criticalcssoption.php
...
/**
 * Criticalcssoption Form Field class for the Pro_critical component
 */
class JFormFieldCriticalcssoption extends JFormFieldList
{
	/**
	 * The criticalcssoption field type.
	 *
	 * @var		string
	 */
	public $type = 'criticalcssoption';
...
// Get the user object.
		...
		$query->select($db->quoteName(array('a.id','a.copmonent_name'),array('id','option_copmonent_name')));
		...
		if ($items)
		{
			...
			{
				$options[] = JHtml::_('select.option', $item->id, $item->option_copmonent_name);
			}
		}
		return $options;
	}
