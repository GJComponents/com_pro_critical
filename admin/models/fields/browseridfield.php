...
/**
 * Browseridfield Form Field class for the Pro_critical component
 */
class JFormFieldBrowseridfield extends JFormFieldList
{
	/**
	 * The browseridfield field type.
	 *
	 * @var		string
	 */
	public $type = 'browseridfield';
	...
	protected function getInput()
	{
		...
		if ($set_button === 'true')
		{
			...
			// [Fields 3456] only add if user allowed to create user_agent_browser
			if ($user->authorise('core.create', 'com_pro_critical') && $app->isAdmin()) // TODO for now only in admin area.
			{
				...
				$button[] = '<a id="'.$button_code_name.'Create" class="btn btn-small btn-success hasTooltip" title="'.JText::sprintf('COM_PRO_CRITICAL_CREATE_NEW_S', $button_label).'" style="border-radius: 0px 4px 4px 0px; padding: 4px 4px 4px 7px;"
					href="index.php?option=com_pro_critical&amp;view=user_agent_browser&amp;layout=edit'.$ref.'" >
					<span class="icon-new icon-white"></span></a>';
			}
			// [Fields 3472] only add if user allowed to edit user_agent_browser
				...
				$script[] = "
					...
					function ".$button_code_name."Button(value) {
						if (value > 0) {
							...
							var url = 'index.php?option=com_pro_critical&view=user_agent_browser_list&task=user_agent_browser.edit&id='+value+'".$refJ."';
			...
			// [Fields 3513] check if button was created for user_agent_browser field.
...
// Get the user object.
		...
		$query->select($db->quoteName(array('a.id','a.name'),array('id','browser_id_list_name')));
		...
		{
			$columns = $db->getTableColumns('#__pro_critical_user_agent_browser');
		...
		if ($items)
		{
			...
			{
				$options[] = JHtml::_('select.option', $item->id, $item->browser_id_list_name);
			}
		}
		return $options;
	}
