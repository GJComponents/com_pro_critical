...
/-------------------------------------------------------------------------------------------------------/
	...
	@package		proCritical
	@subpackage		criticalcssview.php
...
/**
 * Criticalcssview Form Field class for the Pro_critical component
 */
class JFormFieldCriticalcssview extends JFormFieldList
{
	/**
	 * The criticalcssview field type.
	 *
	 * @var		string
	 */
	public $type = 'criticalcssview';
	...
	protected function getInput()
	{
		...
		if ($set_button === 'true')
		{
			...
			// [Fields 3456] only add if user allowed to create directory_views
			if ($user->authorise('core.create', 'com_pro_critical') && $app->isAdmin()) // TODO for now only in admin area.
			{
				...
				$button[] = '<a id="'.$button_code_name.'Create" class="btn btn-small btn-success hasTooltip" title="'.JText::sprintf('COM_PRO_CRITICAL_CREATE_NEW_S', $button_label).'" style="border-radius: 0px 4px 4px 0px; padding: 4px 4px 4px 7px;"
					href="index.php?option=com_pro_critical&amp;view=directory_views&amp;layout=edit'.$ref.'" >
					<span class="icon-new icon-white"></span></a>';
			}
			// [Fields 3472] only add if user allowed to edit directory_views
				...
				$script[] = "
					...
					function ".$button_code_name."Button(value) {
						if (value > 0) {
							...
							var url = 'index.php?option=com_pro_critical&view=directory_views_list&task=directory_views.edit&id='+value+'".$refJ."';
			...
			// [Fields 3513] check if button was created for directory_views field.
...
// Get the user object.
		...
		$query->select($db->quoteName(array('a.id','a.view_component'),array('id','view_view_component')));
		...
		if ($items)
		{
			...
			{
				$options[] = JHtml::_('select.option', $item->id, $item->view_view_component);
			}
		}
		return $options;
	}
