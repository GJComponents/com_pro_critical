...
/**
 * Typedeviceid Form Field class for the Pro_critical component
 */
class JFormFieldTypedeviceid extends JFormFieldList
{
	/**
	 * The typedeviceid field type.
	 *
	 * @var		string
	 */
	public $type = 'typedeviceid';
	...
	protected function getInput()
	{
		...
		if ($set_button === 'true')
		{
			...
			// [Fields 4857] only add if user allowed to create type_device
			...
			// [Fields 4893] only add if user allowed to edit type_device
				...
				$script[] = "
					...
					function ".$button_code_name."Button(value) {
						if (value > 0) {
							...
							var url = 'index.php?option=com_pro_critical&view=type_device_list&task=type_device.edit&id='+value+'".$refJ."';
			...
			// [Fields 4968] check if button was created for type_device field.
...
// Get the user object.
		...
		$query->select($db->quoteName(array('a.id','a.type_device'),array('id','type_device_id_type_device')));
		$query->from($db->quoteName('#__pro_critical_type_device', 'a'));
		$query->where($db->quoteName('a.published') . ' = 1');
		$query->order('a.type_device ASC');
		// Implement View Level Access (if set in table)
		if (!$user->authorise('core.options', 'com_pro_critical'))
		{
			$columns = $db->getTableColumns('#__pro_critical_type_device');
		...
		if ($items)
		{
			...
			{
				$options[] = JHtml::_('select.option', $item->id, $item->type_device_id_type_device);
			}
		}
		return $options;
	}
