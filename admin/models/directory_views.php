...
/-------------------------------------------------------------------------------------------------------/
	...
	@package		proCritical
	@subpackage		directory_views.php
...
/**
 * Pro_critical Directory_views Model
 */
class Pro_criticalModelDirectory_views extends JModelAdmin
{
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_pro_critical.directory_views';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'directory_views', $prefix = 'Pro_criticalTable', $config = array())
	{
...
#Custom Buttons PHP (model methods)
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			{
				// [Interpretation 4715] Convert the query_params field to an array.
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_pro_critical.directory_views');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_pro_critical.directory_views', 'directory_views', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_pro_critical.directory_views.' . (int) $id))
	...
	{
		return 'administrator/components/com_pro_critical/models/forms/directory_views.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_pro_critical.directory_views.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit.state', 'com_pro_critical.directory_views.' . (int) $recordId);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_pro_critical.directory_views.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_pro_critical.edit.directory_views.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// run the perprocess of the data
			$this->preprocessData('com_pro_critical.directory_views', $data);
		}
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		while (!empty($pks))
		{
			...
			// [Interpretation 6023] Only for strings
			if (Pro_criticalHelper::checkString($this->table->view_component) && !is_numeric($this->table->view_component))
			{
				$this->table->view_component = $this->generateUniqe('view_component',$this->table->view_component);
			}
...
########################
### PHP save Method before => Справочник Views
### Target (array) $data values
########################
		
		$query_params_clone = $data['query_params'] ;
		
		
		usort($query_params_clone, function($a, $b) {
			$res = strcmp( $a['param_query'] , $b['param_query'] );
			return $res ;
		});
		
		# Добавляем только уникальные параметры запросов
		$x = 0 ;
		$controlDuble = [] ;
		$data['query_params'] = [];
		foreach( $query_params_clone  as $i => $item )
		{
			if( empty( $item['param_query'] ) ) continue ; #END IF
			if( in_array( $item['param_query'] ,  $controlDuble ) ) continue ; #END IF
			$data['query_params'][ 'query_params' . $x ] = $item ;
			$controlDuble[] =  $item['param_query'] ;
			$x++;
		}#END FOREACH

		
########################
### END PHP save Method before => Справочник Views
########################

		// [Interpretation 4780] Set the query_params items to data.
		...
		{
			// [Interpretation 4804] Set the empty query_params to data
			$data['query_params'] = '';
		}
	...
	protected function _generateNewTitle($title)
	{
		...
		// [Interpretation 6306] Alter the title
		$table = $this->getTable();
