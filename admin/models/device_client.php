...
/**
 * Pro_critical Device_client Model
 */
class Pro_criticalModelDevice_client extends JModelAdmin
{
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_pro_critical.device_client';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'device_client', $prefix = 'Pro_criticalTable', $config = array())
	{
...
/*
######################################
# PHP getItem Method Target (object) $item values.
# File - /administrator/components/com_pro_critical/models/device_client.php
# Prefix - php_getitem
######################################
*/
...
/*php_getitem<=TRACK]]]*/
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_pro_critical.device_client');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_pro_critical.device_client', 'device_client', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_pro_critical.device_client.' . (int) $id))
	...
	{
		return 'administrator/components/com_pro_critical/models/forms/device_client.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_pro_critical.device_client.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit.state', 'com_pro_critical.device_client.' . (int) $recordId);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_pro_critical.device_client.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__pro_critical_device_client'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_pro_critical.edit.device_client.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// run the perprocess of the data
			$this->preprocessData('com_pro_critical.device_client', $data);
		}
...
/*[[[TRACK=>php_before_save*/
		/*
		######################################
		# PHP save Method beforeIn  Target (array) $data values
		# File - /administrator/components/com_pro_critical/models/device_client.php
		# Prefix - php_before_save
		######################################
		*/
		$ignoreFieldArr = [ 'hache', 'id', 'created', 'created_by', 'published', 'modified', 'ordering', 'version', 'asset_id', 'tags' ];
		$hash           = Pro_criticalHelper::getHacheArr( $data, $ignoreFieldArr );
		$res            = Pro_criticalHelper::getVar( 'device_client', '' . $hash, 'hache' );
		
		if ( $res )
		{
			$app = \JFactory::getApplication();
			$app->enqueueMessage( 'Такие параметры уже есть в справочнике', 'WARNING' );
