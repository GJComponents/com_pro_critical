...
/-------------------------------------------------------------------------------------------------------/
	...
	@package		proCritical
	@subpackage		type_device_list.php
...
/**
 * Type_device_list Model
 */
class Pro_criticalModelType_device_list extends JModelList
{
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__pro_critical_type_device'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__pro_critical_type_device'))->set($fields)->where($conditions); 
