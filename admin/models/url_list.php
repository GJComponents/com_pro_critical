 ...
 * Url_list Model
 */
class Pro_criticalModelUrl_list extends JModelList
{
...
#Custom Buttons PHP List view (model methods) [url]
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__pro_critical_url'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__pro_critical_url'))->set($fields)->where($conditions); 
