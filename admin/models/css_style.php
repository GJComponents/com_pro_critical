...
class Pro_criticalModelCss_style extends JModelAdmin
{
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_pro_critical.css_style';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'css_style', $prefix = 'Pro_criticalTable', $config = array())
	{
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_pro_critical.css_style');
			}
		}
	...
	{
		return 'administrator/components/com_pro_critical/models/forms/css_style.js';
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__pro_critical_css_style'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_pro_critical.edit.css_style.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// run the perprocess of the data
			$this->preprocessData('com_pro_critical.css_style', $data);
		}
