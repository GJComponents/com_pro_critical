...
/-------------------------------------------------------------------------------------------------------/
	...
	@package		proCritical
	@subpackage		user_agent_os_list.php
...
/**
 * User_agent_os_list Model
 */
class Pro_criticalModelUser_agent_os_list extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		// [Interpretation 14911] Filter by Code.
		if ($code = $this->getState('filter.code'))
		{
			$query->where('a.code = ' . $db->quote($db->escape($code)));
		}
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__pro_critical_user_agent_os'))->set($fields)->where($conditions); 
