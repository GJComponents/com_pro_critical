...
/-------------------------------------------------------------------------------------------------------/
	...
	@package		proCritical
	@subpackage		directory_components_list.php
...
/**
 * Directory_components_list Model
 */
class Pro_criticalModelDirectory_components_list extends JModelList
{
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
        {
			$config['filter_fields'] = array(
				...
				'a.copmonent_name','copmonent_name'
			);
		}
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$copmonent_name = $this->getUserStateFromRequest($this->context . '.filter.copmonent_name', 'filter_copmonent_name');
		$this->setState('filter.copmonent_name', $copmonent_name);
	...
	protected function getListQuery()
	{
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.copmonent_name LIKE '.$search.')');
			}
		}

		// [Interpretation 14911] Filter by Copmonent_name.
		if ($copmonent_name = $this->getState('filter.copmonent_name'))
		{
			$query->where('a.copmonent_name = ' . $db->quote($db->escape($copmonent_name)));
		}
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':' . $this->getState('filter.copmonent_name');
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__pro_critical_directory_components'))->set($fields)->where($conditions); 
