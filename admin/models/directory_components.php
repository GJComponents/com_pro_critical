...
/-------------------------------------------------------------------------------------------------------/
	...
	@package		proCritical
	@subpackage		directory_components.php
...
/**
 * Pro_critical Directory_components Model
 */
class Pro_criticalModelDirectory_components extends JModelAdmin
{
	/**
	 ...
	 * @since    3.2
	 */
	public $typeAlias = 'com_pro_critical.directory_components';
	 ...
	 * @since   1.6
	 */
	public function getTable($type = 'directory_components', $prefix = 'Pro_criticalTable', $config = array())
	{
...
#Custom Buttons PHP (model methods)[directory_components]
	...
	public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk))
		{
			...
			if (!empty($item->id))
			{
				...
				$item->tags->getTagIds($item->id, 'com_pro_critical.directory_components');
			}
		}
	...
	public function getForm($data = array(), $loadData = true, $options = array('control' => 'jform'))
	{
		...
		$form = $this->loadForm('com_pro_critical.directory_components', 'directory_components', $options, $clear, $xpath);
		...
		if ($id != 0 && (!$user->authorise('core.edit.state', 'com_pro_critical.directory_components.' . (int) $id))
	...
	{
		return 'administrator/components/com_pro_critical/models/forms/directory_components.js';
	}
	...
	protected function canDelete($record)
	{
		if (!empty($record->id))
		{
			...
			return $user->authorise('core.delete', 'com_pro_critical.directory_components.' . (int) $record->id);
		}
		return false;
	}
	...
	protected function canEditState($record)
	{
		...
		if ($recordId)
		{
			...
			$permission = $user->authorise('core.edit.state', 'com_pro_critical.directory_components.' . (int) $recordId);
	...
	protected function allowEdit($data = array(), $key = 'id')
	{
		...
		return JFactory::getUser()->authorise('core.edit', 'com_pro_critical.directory_components.'. ((int) isset($data[$key]) ? $data[$key] : 0)) or parent::allowEdit($data, $key);
	}
		...
		if (empty($table->id))
		{
			...
			if (empty($table->ordering))
			{
				...
				$query = $db->getQuery(true)
					->select('MAX(ordering)')
					->from($db->quoteName('#__pro_critical_directory_components'));
				$db->setQuery($query);
				$max = $db->loadResult();
	...
	protected function loadFormData() 
	{
		...
		$data = JFactory::getApplication()->getUserState('com_pro_critical.edit.directory_components.data', array());

		if (empty($data))
		{
			$data = $this->getItem();
			// run the perprocess of the data
			$this->preprocessData('com_pro_critical.directory_components', $data);
		}
	...
	protected function batchCopy($values, $pks, $contexts)
	{
		...
		while (!empty($pks))
		{
			...
			if (Pro_criticalHelper::checkString($this->table->copmonent_name) && !is_numeric($this->table->copmonent_name))
			{
				$this->table->copmonent_name = $this->generateUniqe('copmonent_name',$this->table->copmonent_name);
			}
