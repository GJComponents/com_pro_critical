 ...
 * Admins_list Model
 */
class Pro_criticalModelAdmins_list extends JModelList
{
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$admin_name = $this->getUserStateFromRequest($this->context . '.filter.admin_name', 'filter_admin_name');
		$this->setState('filter.admin_name', $admin_name);
	...
	protected function getListQuery()
	{
		...
		$query->from($db->quoteName('#__pro_critical_admins', 'a'));
		...
		if (!empty($search))
		{
			...
			else
			{
				...
				$query->where('(a.admin_name LIKE '.$search.')');
			}
		}
	...
	protected function getStoreId($id = '')
	{
		...
		$id .= ':' . $this->getState('filter.admin_name');
	...
	protected function checkInNow()
	{
		...
		if ($time)
		{
			...
			$query = $db->getQuery(true);
			$query->select('*');
			$query->from($db->quoteName('#__pro_critical_admins'));
			$db->setQuery($query);
			$db->execute();
			if ($db->getNumRows())
			{
				...
				$query->update($db->quoteName('#__pro_critical_admins'))->set($fields)->where($conditions); 
