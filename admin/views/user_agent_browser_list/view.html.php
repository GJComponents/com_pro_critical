...
/**
 * Pro_critical View class for the User_agent_browser_list
 */
class Pro_criticalViewUser_agent_browser_list extends JViewLegacy
{
	/**
	 * User_agent_browser_list view display method
	 * @return void
	 */
	function display($tpl = null)
	{
		if ($this->getLayout() !== 'modal')
		{
			...
			Pro_criticalHelper::addSubmenu('user_agent_browser_list');
		}
		...
		$this->canDo = Pro_criticalHelper::getActions('user_agent_browser');
	...
	{
		JToolBarHelper::title(JText::_('COM_PRO_CRITICAL_USER_AGENT_BROWSER_LIST'), 'joomla');
		JHtmlSidebar::setAction('index.php?option=com_pro_critical&view=user_agent_browser_list');
		JFormHelper::addFieldPath(JPATH_COMPONENT . '/models/fields');

		if ($this->canCreate)
		{
			JToolBarHelper::addNew('user_agent_browser.add');
		}
		...
		if (Pro_criticalHelper::checkArray($this->items))
		{
			if ($this->canEdit)
			{
				JToolBarHelper::editList('user_agent_browser.edit');
			}

			if ($this->canState)
			{
				JToolBarHelper::publishList('user_agent_browser_list.publish');
				JToolBarHelper::unpublishList('user_agent_browser_list.unpublish');
				JToolBarHelper::archiveList('user_agent_browser_list.archive');

				if ($this->canDo->get('core.admin'))
				{
					JToolBarHelper::checkin('user_agent_browser_list.checkin');
				}
			}
			...
			{
				JToolbarHelper::deleteList('', 'user_agent_browser_list.delete', 'JTOOLBAR_EMPTY_TRASH');
			}
			elseif ($this->canState && $this->canDelete)
			{
				JToolbarHelper::trash('user_agent_browser_list.trash');
			}
		}

		// set help url for this view if found
		$help_url = Pro_criticalHelper::getHelpUrl('user_agent_browser_list');
		...
		if (Pro_criticalHelper::checkArray($this->versionOptions))
		{
			...
			JHtmlSidebar::addFilter(
				...
				'filter_version',
				JHtml::_('select.options', $this->versionOptions, 'value', 'text', $this->state->get('filter.version'))
			);
			...
			if ($this->canBatch && $this->canCreate && $this->canEdit)
			{
				...
				JHtmlBatch_::addListSelection(
					...
					'batch[version]',
					JHtml::_('select.options', $this->versionOptions, 'value', 'text')
				);
			}
		}
	}
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_('COM_PRO_CRITICAL_USER_AGENT_BROWSER_LIST'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_pro_critical/assets/css/user_agent_browser_list.css", (Pro_criticalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
	}
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.name' => JText::_('COM_PRO_CRITICAL_USER_AGENT_BROWSER_NAME_LABEL'),
			'a.version' => JText::_('COM_PRO_CRITICAL_USER_AGENT_BROWSER_VERSION_LABEL'),
