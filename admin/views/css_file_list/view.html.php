...
/**
 * Pro_critical View class for the Css_file_list
 */
class Pro_criticalViewCss_file_list extends JViewLegacy
{
	/**
	 * Css_file_list view display method
	 * @return void
	 */
	function display($tpl = null)
	{
		if ($this->getLayout() !== 'modal')
		{
			...
			Pro_criticalHelper::addSubmenu('css_file_list');
		}
	...
	{
		JToolBarHelper::title(JText::_('COM_PRO_CRITICAL_CSS_FILE_LIST'), 'joomla');
		JHtmlSidebar::setAction('index.php?option=com_pro_critical&view=css_file_list');
		JFormHelper::addFieldPath(JPATH_COMPONENT . '/models/fields');

		if ($this->canCreate)
		{
			JToolBarHelper::addNew('css_file.add');
		}
		...
		if (Pro_criticalHelper::checkArray($this->items))
		{
			if ($this->canEdit)
			{
				JToolBarHelper::editList('css_file.edit');
			}

			if ($this->canState)
			{
				JToolBarHelper::publishList('css_file_list.publish');
				JToolBarHelper::unpublishList('css_file_list.unpublish');
				JToolBarHelper::archiveList('css_file_list.archive');

				if ($this->canDo->get('core.admin'))
				{
					JToolBarHelper::checkin('css_file_list.checkin');
				}
			}
			...
			{
				JToolbarHelper::deleteList('', 'css_file_list.delete', 'JTOOLBAR_EMPTY_TRASH');
			}
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_('COM_PRO_CRITICAL_CSS_FILE_LIST'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_pro_critical/assets/css/css_file_list.css", (Pro_criticalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
	}
	...
	protected function getSortFields()
	{
		return array(
			...
			'a.file' => JText::_('COM_PRO_CRITICAL_CSS_FILE_FILE_LABEL'),
			'a.load' => JText::_('COM_PRO_CRITICAL_CSS_FILE_LOAD_LABEL'),
