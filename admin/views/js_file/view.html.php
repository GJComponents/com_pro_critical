...
class Pro_criticalViewJs_file extends JViewLegacy
{
	...
	protected function addToolBar()
	{
		...
		JToolbarHelper::title( JText::_($isNew ? 'COM_PRO_CRITICAL_JS_FILE_NEW' : 'COM_PRO_CRITICAL_JS_FILE_EDIT'), 'pencil-2 article-add');
		...
		if (Pro_criticalHelper::checkString($this->referral))
		{
			if ($this->canDo->get('core.create') && $isNew)
			{
				...
				JToolBarHelper::save('js_file.save', 'JTOOLBAR_SAVE');
			}
			...
			if ($isNew)
			{
				...
				JToolBarHelper::cancel('js_file.cancel', 'JTOOLBAR_CANCEL');
			}
			else
			{
				// [Interpretation 12603] We can close it.
				JToolBarHelper::cancel('js_file.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		else
		{
			if ($isNew)
			{
				...
				{
					JToolBarHelper::apply('js_file.apply', 'JTOOLBAR_APPLY');
					JToolBarHelper::save('js_file.save', 'JTOOLBAR_SAVE');
					JToolBarHelper::custom('js_file.save2new', 'save-new.png', 'save-new_f2.png', 'JTOOLBAR_SAVE_AND_NEW', false);
				};
				JToolBarHelper::cancel('js_file.cancel', 'JTOOLBAR_CANCEL');
			}
			else
			{
				...
				if ($this->canDo->get('core.create'))
				{
					JToolBarHelper::custom('js_file.save2copy', 'save-copy.png', 'save-copy_f2.png', 'JTOOLBAR_SAVE_AS_COPY', false);
				}
				JToolBarHelper::cancel('js_file.cancel', 'JTOOLBAR_CLOSE');
			}
		}
		...
		$help_url = Pro_criticalHelper::getHelpUrl('js_file');
	...
	protected function setDocument()
	{
		...
		$this->document->setTitle(JText::_($isNew ? 'COM_PRO_CRITICAL_JS_FILE_NEW' : 'COM_PRO_CRITICAL_JS_FILE_EDIT'));
		$this->document->addStyleSheet(JURI::root() . "administrator/components/com_pro_critical/assets/css/js_file.css", (Pro_criticalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/css');
		...
		$this->document->addScript(JURI::root() . "administrator/components/com_pro_critical/views/js_file/submitbutton.js", (Pro_criticalHelper::jVersion()->isCompatible('3.8.0')) ? array('version' => 'auto') : 'text/javascript'); 
