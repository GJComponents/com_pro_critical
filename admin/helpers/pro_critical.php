...
######PHP (admin helper) *######
		/**
		 * @param $name - Имя компонента. Параметр GET - option
		 * @return array - Extension Info
		 * @since 3.9
		 */
		public static function getExtension( $name , $type = 'component'){
			$db =    JFactory::getDBO();
...
//			 echo 'Query Dump :'.__FILE__ .' Line:'.__LINE__  .$query->dump() ;
         ...
         /**
	 * Выборка из MAP таблицы
	 * @param $table - Имя таблицы '#__pro_critical_'.$table
	 * @param $key  - Имя поля - Ключ связи
	 * @param $id  - Значение ключа
	 * @param $select
	 * @return mixed
	 * @since 3.9
	 */
	public static function getDataMapTable ( $table , $key , $id , $select ){
		$db = JFactory::getDbo();
		$query = $db->getQuery( true );
		$_sqlTable = '#__pro_critical_'.$table ;
		
		$query->select( $db->quoteName($select) )
			->from( $db->quoteName($_sqlTable) )
		...
		return  $db->loadColumn();
         ...
         /**
	 * Удалить связи из таблиц MAP
	 * @param $table - Имя таблицы '#__pro_critical_'.$table
	 * @param array| int $key   - Имя поля - Ключ связи
	 * @param $id    - Значение ключа
	 * @since 3.9
         * @return bool
	 */
	public static function deliteDataMapTable ( $table , $key , $id ){
		$db = JFactory::getDbo();
		$query = $db->getQuery( true );
		$_sqlTable = '#__pro_critical_'.$table ;
		...
		foreach( $pks as $item )
		{
			$conditions[] = $db->quoteName(''.$key ) . ' =  ' .$db->quote( $item ) ;
		}#END FOREACH
		
		$query->where($conditions , $glue = 'OR');
		$query->delete( $db->quoteName( $_sqlTable ));
		// echo $query->dump() ;
	...
	/**
	 * Добавить данные в таблицы MAP
         * 
	 * @return mixed
	 * @since 3.9
	 */
	public static function addDataMapTable ( $table , $key  , $columns ,  $id  , $asInsert = []  )
	{
		Pro_criticalHelper::deliteDataMapTable( $table , $key , $id ) ;
		$db = JFactory::getDbo();
		$query = $db->getQuery( true );
 
		$_sqlTable = '#__pro_critical_'.$table ;

		foreach ( $asInsert as $item )
		{
			$values =
				$db->quote( $id ) . ","
				. $db->quote( $item ) ;
		...
		$query->insert( $db->quoteName( $_sqlTable ) )->columns( $db->quoteName( $columns ) );
		$db->setQuery( $query );
//		echo $query->dump();
		$db->execute();
		return true ;
	...
	/**
	 * Получить хешь из массива
	 * @param array $arr - исходный массив
	 * @param array $ignoreKeys - ключи которые игнарировать
	 * @return string
	 * @since 3.9
	 */
	public static function getHacheArr( $arr , $ignoreKeys = [] ){
		
		foreach( $arr as $i => $datum )
		{
			
			if( !in_array( $i , $ignoreKeys ) )
			{
				$datum = (string)$datum;
				$cloneData[ $i ] = $datum;
		...
		ksort( $cloneData );
		reset( $cloneData );
		return md5( json_encode( $cloneData ) );
		
	}#END FN


	/**
	 * Очистить все таблицы в компоненте
	 * @return bool
	 * @throws Exception
	 * @since 3.9
	 */
	public static function CleanAllTable( ){
		
		$prefix = 'pro_critical' ;
		// $arrView = ['css' , 'device_client' , 'url' , 'user_agent' , 'user_agent_browser' ,  'user_agent_os' ];
                $arrView = [ 'css' , 'device_client' , 'url'  , 'user_agent_browser' ,  'user_agent_os' ];
		...
		{
			$db->truncateTable('#__'.$prefix.'_' . $item );
		}#END FOREACH
		$app->enqueueMessage('Таблицы очищенны');
...
###### END PHP (admin helper) *######
	...
	public static function addSubmenu($submenu)
	{
		...
		JHtmlSidebar::addEntry(JText::_('COM_PRO_CRITICAL_SUBMENU_USER_AGENT_BROWSER_LIST'), 'index.php?option=com_pro_critical&view=user_agent_browser_list', $submenu === 'user_agent_browser_list');
		JHtmlSidebar::addEntry(JText::_('COM_PRO_CRITICAL_SUBMENU_DEVICE_CLIENT_LIST'), 'index.php?option=com_pro_critical&view=device_client_list', $submenu === 'device_client_list');
		JHtmlSidebar::addEntry(JText::_('COM_PRO_CRITICAL_SUBMENU_ADMINS_LIST'), 'index.php?option=com_pro_critical&view=admins_list', $submenu === 'admins_list');
		JHtmlSidebar::addEntry(JText::_('COM_PRO_CRITICAL_SUBMENU_HTML_TASK_LIST'), 'index.php?option=com_pro_critical&view=html_task_list', $submenu === 'html_task_list');
		JHtmlSidebar::addEntry(JText::_('COM_PRO_CRITICAL_SUBMENU_TYPE_DEVICE_LIST'), 'index.php?option=com_pro_critical&view=type_device_list', $submenu === 'type_device_list');
		JHtmlSidebar::addEntry(JText::_('COM_PRO_CRITICAL_SUBMENU_TYPE_BROWSER_LIST'), 'index.php?option=com_pro_critical&view=type_browser_list', $submenu === 'type_browser_list');
		JHtmlSidebar::addEntry(JText::_('COM_PRO_CRITICAL_SUBMENU_DIRECTORY_COMPONENTS_LIST'), 'index.php?option=com_pro_critical&view=directory_components_list', $submenu === 'directory_components_list');
	 ...
	 *
	 */
	public static function getVar($table, $where = null, $whereString = 'user', $what = 'id', $operator = '=', $main = 'pro_critical')
	{
		if(!$where)
		{
	 ...
	 *
	 */
	public static function getVars($table, $where = null, $whereString = 'user', $what = 'id', $operator = 'IN', $main = 'pro_critical', $unique = true)
	{
		if(!$where)
		{
			$where = JFactory::getUser()->id;
	...
	public static function isPublished($id,$type)
	{
		...
		$query->from('#__pro_critical_'.$type.' AS a');
	...
	public static function setAsset($id, $table, $inherit = true)
	{
		...
		$parent->loadByName('com_pro_critical');
		
		$parentId = $parent->id;
		$name     = 'com_pro_critical.'.$table.'.'.$id;
		$title    = '';
		...
		else
		{
			...
			$rules = self::getDefaultAssetRules('com_pro_critical', $table, $inherit);
			...
			else
			{
				...
				return JFactory::getDbo()->updateObject('#__pro_critical_'.$table, $object, 'id');
			}
		}
		return false;
	}
